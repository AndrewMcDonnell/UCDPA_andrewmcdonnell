...
#pd.set_option('display.max_columns',10pd.set_option('display.max_rows',400)
...
full = pd.concat([unemp1, unemp2, unemp3], axis=0)
full.set_index('Month', inplace=True)
print(full)

print(len(unemp1) + len(unemp2) + len(unemp3))

#merged = pd.merge(clean_month_list2,unemp_covid_use,how='left', on='Month')

#merged2 = pd.merge(merged,unemp9821_filter,how='left', on='Month')

#merged3= pd.merge(merged2, unemp8315_filter, how='left', on='Month')
